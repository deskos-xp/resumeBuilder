...
Wyoming'''.split('\n')
    ...
    def contact_invalidUntilFilled_email(self,qtype):
        phone=self.phoneStripper(self.lineEdit_3.text())
        if len(self.lineEdit_5.text()) > 0:
            email=checkers_lib.checkers_email()
            verify=email.correct_email(self.lineEdit_5.text())
            if verify[1] == True:
